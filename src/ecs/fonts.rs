...
thread_local! {
	...
	pub static KANA_FONT: RefCell<Option<Object>> = RefCell::new(None);
	pub static KANJI_FONT: RefCell<Option<Object>> = RefCell::new(None);
}

const KANA_FONT_URL: &'static str = "assets/typeface/OsakaRegularMonoEnJaRestrictedReversed.json";
const KANJI_FONT_URL: &'static str = "assets/typeface/mplus-1m-light_1m-light.json";

pub fn with_kana_font(f: impl Fn(&Object) + 'static) {
	with_font(KANA_FONT_URL, &KANA_FONT, f);
}
